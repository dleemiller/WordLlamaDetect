...
def eval_command(args):
    ...
    # Check which mode: fp8 inference or PyTorch training
    if args.model_path:
        # FP8 inference mode - minimal dependencies
        ...
        logger.info("=" * 60)
        logger.info("WLDETECT FP8 EVALUATION")
        logger.info("=" * 60)
        ...
        logger.info(
            f"Model: {detector.config.model.name if detector.config.model else 'Multi-model'}"
        )
        ...
        # PyTorch training mode - requires training dependencies
        import torch
        ...
        logger.info("=" * 60)
        logger.info("WLDETECT PYTORCH EVALUATION")
        logger.info("=" * 60)
        ...
        embeddings_tensor = (
            torch.from_numpy(embeddings).clone().to(device=device, dtype=target_dtype)
        )
...
def main() -> int:
    ...
    # Mode selection: either --config for PyTorch or --model-path for fp8
