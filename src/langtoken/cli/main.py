...
def setup_logging(level: int = logging.INFO) -> logging.Logger:
    ...
    return logging.getLogger("langtoken")
...
def train_command(args):
    ...
    try:
        ...
        from langtoken.config.loader import load_model_config, load_training_config
    ...
    from langtoken.embeddings.extractor import get_cache_path, load_embeddings_as_memmap
    ...
    from langtoken.training.flores_eval import evaluate_on_flores, save_flores_evaluation
...
def eval_command(args):
    ...
    from langtoken.config.loader import load_model_config, load_training_config
    from langtoken.embeddings.extractor import (
        extract_embeddings,
        get_cache_path,
    ...
    from langtoken.training.flores_eval import save_confusion_heatmap
...
def detect_command(args):
    ...
    detector = LanguageDetector(args.model_path)

    if args.text:
        # Detect single text
        probs = detector.detect(args.text)
        top_lang = detector.get_top_language(args.text)

        logger.info(f"\nText: {args.text}")
        logger.info(f"Detected language: {top_lang}")
        logger.info("\nProbabilities:")
        ...
        top_lang = detector.get_top_language(text)
...
def prune_command(args):
    """Execute language pruning command."""
    logger = setup_logging()
    logger.warning(
        "The 'prune' command has been replaced by 'curate'. Please use 'langtoken curate'."
    )
    return 1
...
def curate_command(args):
    ...
    from langtoken.config.loader import load_model_config, load_training_config, save_model_config
    from langtoken.embeddings.extractor import (
        extract_embeddings,
        get_cache_path,
        load_embeddings_as_memmap,
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="LangToken - Language detection using static LLM embeddings"
    )
